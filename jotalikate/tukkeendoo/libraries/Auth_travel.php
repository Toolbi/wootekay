...
class Auth_travel
{
	...
	*/
	function login_travel($email, $password, $remember=false)
	{
		...
		if (sizeof($result) > 0)
		{
			...
			if ($result['user_token'] == null) {
				...
				$token	= sha1(random_string('alnum', 30));
			}else{
	...
	this function does the logging in.
	*/
	function login_travel_by_token($token, $remember=false)
	{
		$this->CI->db->select('*');
		...
		if (sizeof($result) > 0)
		{
			...
			$carpool['carpool']['user_id']	= $result['user_id'];
			$carpool['carpool']['trip_id']	= $this->trips($result['user_id']);
			$carpool['carpool']['token']	= $result['user_token'];
